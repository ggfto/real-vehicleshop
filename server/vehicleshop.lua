CreateThread(
    function()
        RegisterCallback(
            "real-vehicleshop:GetVehicleshopData",
        ...
        RegisterCallback(
            "real-vehicleshop:RemoveMoneyForTestDrive",
            function(source, cb)
                local src = source
                local PlayerBank = GetPlayerMoneyOnline(src, "bank")
        ...
        RegisterCallback(
            "real-vehicleshop:CheckPlateStatus",
        ...
        RegisterCallback(
            "real-vehicleshop:BuyPlayerVehicle",
            function(source, cb, data)
                ...
                if PlayerBank >= vehicleprice then
                    if Config.Vehicleshops[data.id].Owner == "" then
                        ...
                        local result =
                            ExecuteSql(
                            "SELECT `information`, `vehicles` FROM `real_vehicleshop` WHERE `id` = '" .. data.id .. "'"
                        )
                        if #result > 0 then
                            ...
                            if Check then
                                ...
                                Config.Vehicleshops[data.id].CompanyMoney =
                                    Config.Vehicleshops[data.id].CompanyMoney + vehicleprice
...
RegisterNetEvent(
    "real-vehicleshop:TestDrive",
    function(started, netid)
        local src = source
        if started then
...
RegisterNetEvent(
    "real-vehicleshop:PreOrderVehicle",
    function(data, props)
        local src = source
        local result =
        ...
        if #result > 0 then
            ...
            if PlayerBank >= vehicleprice then
                ...
                TriggerClientEvent(
                    ...
                    src,
                    "success",
                    Language("preorder_request_sent"),
                    3000
                )
            end
        end
            ...
            if next(preorders) then
                for k, v in ipairs(preorders) do
                    if os.time() >= v.expiretime then
                        ...
                        SendMailToOfflinePlayer(
                            v.identifier,
                            Config.Vehicleshops[shopId].CompanyName,
                            Language("preorder_rejected_subject"),
                            Language("preorder_rejected_message")
                        )
                ...
                if Check then
                    ...
                    ExecuteSql(
                        "UPDATE `real_vehicleshop` SET `information` = '" ..
                            json.encode(information) ..
                                "', `preorders` = '" .. json.encode(preorders) .. "' WHERE `id` = '" .. shopId .. "'"
                    )
                    TriggerClientEvent("real-vehicleshop:Update", -1, Config.Vehicleshops)
                end
            end
