...
function AddBankMoneyOffline(identifier, payment)
    if Config.Framework == "qb" or Config.Framework == "oldqb" then
        ...
        ExecuteSql(
            "UPDATE players SET money = '" .. json.encode(targetMoney) .. "' WHERE citizenid = '" .. identifier .. "'"
        )
    else
        ...
        targetMoney.bank = targetMoney.bank + payment
        ExecuteSql(
            "UPDATE users SET accounts = '" .. json.encode(targetMoney) .. "' WHERE identifier = '" .. identifier .. "'"
        )
    end
end
